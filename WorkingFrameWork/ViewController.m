...
@interface ViewController()<NSTextFieldDelegate>
{
    ...
    __weak IBOutlet NSTextField *network_State_TF;
    __weak IBOutlet NSTextField *Server_State_TF;
...
- (void)viewDidLoad {
    ...
    //初始化温湿度和主控板
     ...
     totalTimes_Foam = [[[NSUserDefaults standardUserDefaults] objectForKey:kFoamNum] intValue];
...
#pragma mark------------//index=7,判断当前配置文件和changeID等配置
        if (index == 7) { //判断当前配置文件和changeID等配置
            ...
            {
                [Status_TF setStringValue:@"index = 7,双击启动按钮"];
                index = 8;
                //startbutton.enabled = NO;
            }
            //*********************end*********************//
        ...
        if (fix_A_num == 101) {
            ...
            if (param.isDebug) {
                ...
                if (testnum== 4||testnum ==[ChooseNumArray count]) {
                 ...
                 [self LightAndShowResultWithFix:notiString_A withTextField:NS_TF1 TestingFixStr:testingFixStr Dictionary:A_resultDic];
            }
            else
            {
                [self LightAndShowResultWithFix:notiString_A withTextField:NS_TF1 TestingFixStr:testingFixStr Dictionary:A_resultDic];
...
#pragma mark-------------//index=102,B治具测试结束，发送指令信号灯
        if (fix_B_num == 102) {
            ...
            else
            {
              ...
              [self LightAndShowResultWithFix:notiString_B withTextField:NS_TF2 TestingFixStr:testingFixStr Dictionary:B_resultDic];
            }
...
#pragma mark-------------//index=103,C治具测试结束，发送指令信号灯
        if (fix_C_num == 103) {
            ...
            {
                [self LightAndShowResultWithFix:notiString_C withTextField:NS_TF3 TestingFixStr:testingFixStr Dictionary:C_resultDic];
            }
        }
...
#pragma mark-------------//index=104,C治具测试结束，发送指令信号灯
        if (fix_D_num == 104) { //扫描SN
            ...
            {
               [self LightAndShowResultWithFix:notiString_D withTextField:NS_TF4 TestingFixStr:testingFixStr Dictionary:D_resultDic];
            }
...
#pragma mark-------------//index=105,所有软件测试结束
        if (index == 105) {
            ...
            if (Test_Foam_Num>=totalTimes_Foam){
                [alert ShowCancelAlert:[NSString stringWithFormat:@"泡棉已达到%d,请更换泡棉",totalTimes_Foam]];
            }
            //存在本地
            [[NSUserDefaults standardUserDefaults] setValue:[NSString stringWithFormat:@"%d",Test_Foam_Num>=totalTimes_Foam?0:Test_Foam_Num] forKey:kPresentNum];
...
-(void)LightAndShowResultWithFix:(NSString *)notiString withTextField:(NSTextField *)tf TestingFixStr:(NSString *)testingFix Dictionary:(NSDictionary *)resultDic
{
    ...
    else//数据保存在本地
    {
        @autoreleasepool {
            ...
            [urlString appendFormat:@"%@=%@&", SFC_TEST_SN, tf.stringValue];
        ...
        if (status == AFNetworkReachabilityStatusReachableViaWWAN || status == AFNetworkReachabilityStatusReachableViaWiFi) {
            ...
            if(isExist){
                ...
                for (int i=0;i<[dataArr count]-1;i++) {
                    [uploadManager UploadDataFromLocalFile:[dataArr objectAtIndex:i]];
                }
                //删掉数据
